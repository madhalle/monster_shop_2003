  ...
  describe "when I visit my profile/orders page" do
    ...
    it "I can see every order I've made" do
      ...
      within("#order-#{@orders[0].id}") do
        ...
        expect(page).to have_content("Unique Items: #{@orders[0].unique_item_count}")
        expect(page).to have_content("Total Items: #{@orders[0].total_item_count}")
      end
      within("#order-#{@orders[1].id}") do
        ...
        expect(page).to have_content("Unique Items: #{@orders[1].unique_item_count}")
        expect(page).to have_content("Total Items: #{@orders[1].total_item_count}")
      end
      within("#order-#{@orders[2].id}") do
        ...
        expect(page).to have_content("Unique Items: #{@orders[2].unique_item_count}")
        expect(page).to have_content("Total Items: #{@orders[2].total_item_count}")
      end
    end
  end
end
